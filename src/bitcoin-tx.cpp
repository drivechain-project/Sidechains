...
static void MutateTxSign(CMutableTransaction& tx, const std::string& flagStr)
{
	...
	//TODO: This is obviously wrong, need to fix later
	const CTransactionRef& coinbaseTx = MakeTransactionRef(tx);
        SignatureData sigdata;
        // Only sign SIGHASH_SINGLE if there's a corresponding output:
        if (!fHashSingle || (i < mergedTx.vout.size()))
            ProduceSignature(MutableTransactionSignatureCreator(&keystore, &mergedTx, i, amount,coinbaseTx, nHashType), prevPubKey, sigdata);

        // ... and merge in other signatures:
        BOOST_FOREACH(const CTransaction& txv, txVariants)
            sigdata = CombineSignatures(prevPubKey, MutableTransactionSignatureChecker(&mergedTx, i, amount, coinbaseTx), sigdata, DataFromTransaction(txv, i));
        UpdateTransaction(mergedTx, i, sigdata);
        if (!VerifyScript(txin.scriptSig, prevPubKey, &txin.scriptWitness, STANDARD_SCRIPT_VERIFY_FLAGS, MutableTransactionSignatureChecker(&mergedTx, i, amount, coinbaseTx)))
            fComplete = false;
    }
